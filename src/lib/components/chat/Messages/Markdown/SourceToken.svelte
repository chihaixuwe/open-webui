...
{#if sourceIds}
	{#if (token?.ids ?? []).length == 1}
		{@const sourceId = token.ids[0]}
		{@const identifier = token.citationIdentifiers ? token.citationIdentifiers[0] : sourceId - 1}
		<Source id={identifier} title={sourceIds[sourceId - 1]} {onClick} />
	{:else}
